...
def cargar_pacientes (pacientes:list[list]):
    ...
    for i in range (cantidad_a_ingresar):
        ...
        dias_de_internacion = input(f"¿Cuántos días de internacion lleva el {i+1}° paciente?: ")
        while dias_de_internacion.isdigit() == False or int(dias_de_internacion) < 0:
            dias_de_internacion = input(f"ERROR.¿Cuántos días de internacion lleva el {i+1}° paciente?: ")
...
def buscar_paciente_x_historia_clinica (pacientes:list[list]):
    ...
    """
    encontrado = False
    paciente_buscado = int(input ("Ingrese el numero de historia clínica del paciente: "))
    for i in range (len(pacientes)):
...
def ordenar_pacientes_por_número (pacientes:list[list]):
...
def encontrar_paciente_mas_dias (pacientes:list[list]):
    ...
    le diagnosticaron {diagnostico_paciente_mas_dias} y está internado hace {paciente_mas_dias} días """)        

def paciente_menos_dias (pacientes:list[list]):
    ...
    le diagnosticaron {diagnostico_paciente_menos_dias} y está internado hace {paciente_menos_dias} días """)
...
def calcular_promedio_internacion(pacientes:list[list]):
    ...
    promedio_tiempo_internacion = round(promedio_tiempo_internacion,2)


    print (f"El tiempo promedio de internación es {promedio_tiempo_internacion} días.")